...
SPRING_PROXY = "http://223.130.130.196:8090/api/ai" # Spring 서버 프록시 URL

BUCKET_NAME = "careeyes"
...
def generate_frames(youtube_url, cctv_id='unknown'):
    while True:
        try:
            annotated_img, object_counts, file_url, date_str, time_str, cctv_id = process_youtube_frame(youtube_url, save_result=False, cctv_id=cctv_id)
...
def process_youtube_frame(youtube_url, save_result=False, cctv_id='unknown', delay=DELAY):
    ...
    if save_result and should_save:
        ...
        file_url = upload_to_ncloud(image_stream, f"{cctv_id}/{date_str}/{time_str}.jpg")  # Ncloud에 업로드

        print(f"🔍 탐지 완료! {object_counts} → 저장: {file_url}")

        return annotated_img, object_counts, file_url, date_str, time_str, cctv_id
    else:
        return annotated_img, {}, None, None, None, cctv_id
...
def detect_from_stream():
    try:
        ...
        annotated_img, object_counts, file_url, date_str, time_str, cctv_id = process_youtube_frame(youtube_url, save_result=True)
...
def detect_loop(url, cctv_id='unknown', delay=DELAY):
    while True:
        try:
            annotated_img, object_counts, file_url, date_str, time_str, cctv_id = process_youtube_frame(url, save_result=True, cctv_id=cctv_id, delay=delay)

            # image가 저장되었다면 Spring 서버로 전송
            if file_url and object_counts:
                status, msg = send_to_spring(object_counts, file_url, date_str, time_str, cctv_id)
